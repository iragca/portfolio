---
hide:
  - navigation
  - toc
title: Home
---

# Hi, my name is Chris Irag

I always love building solutions for things that I am passionate about.


## Featured Work

<div class="grid cards" markdown>

-   :fontawesome-solid-graduation-cap:{ .lg .middle } __TRACE__

    ---

    <img src="https://pub-2605a5350a664b7cad96b61a19778380.r2.dev/capstone/website.png">

    My final group capstone project as a **BSc Data Science** student.

    [:octicons-arrow-right-24: Details](projects/data-science/capstone.md)

-   :material-map-outline:{ .lg .middle } __USTP-CDO Map__

    ---


    <img src="https://pub-2605a5350a664b7cad96b61a19778380.r2.dev/map/map.png">

    Interactive 2D map of my university campus.

    [:octicons-arrow-right-24: Details](projects/web-dev/map.md)

</div>


## Currently working on this

<div class="grid cards" markdown>

-   :lucide-hammer:{ .lg .middle } __Docuisine__

    ---

    <img src="https://pub-2605a5350a664b7cad96b61a19778380.r2.dev/docuisine/homepage.png">

    Browse, create and share recipes.

    [:octicons-arrow-right-24: Details](projects/web-dev/docuisine.md)


</div>


## Trackers

[![roadmap.sh](https://roadmap.sh/card/tall/696751d7e3a4245b31c1215f?variant=dark&roadmaps=%2Cdevops%2Cpython%2Cdata-engineer)](https://roadmap.sh/u/iragca)

![Leetcode Stats](https://leetcard.jacoblin.cool/iragca?ext=activity)